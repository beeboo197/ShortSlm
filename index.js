...
break;      
  ...
  if (laz.test(url)){
    const linkLaz = await `https://c.lazada.vn/t/c.06wSoi?intent=true&fallback=false&url=${encodeURIComponent(url)}&sub_aff_id=shorTool`
...
break;
  ...
  console.log(message)
  const voucherCode = await `https://c.lazada.vn/t/c.06wSoi?intent=true&fallback=false&url=${encodeURIComponent(`https://www.lazada.vn/catalog/?q=${message}`)}&sub_aff_id=shorTool`
